...
def process_data(data: Tuple[int, Tuple[Tensor, Tensor, Tensor], str, Dict[str, GreedyGenerator]]) -> Tuple[int, str]:
...
def predict_greedy_raw_multiproc(dataset,
    ...
    with ProcessPoolExecutor() as executor:
        for i, pred in tqdm(executor.map(process_data, [(i, (xyt, kb_tokens, traj_pad_mask), grid_name, grid_name_to_greedy_generator) for i, ((xyt, kb_tokens, _, traj_pad_mask, _), _, grid_name) in enumerate(dataset)]), total=len(dataset)):
            preds[i] = [pred]
