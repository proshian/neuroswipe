...
Очень странно, но посылка yandex-cup не воспроизводится из-за типа данных токенов. Раньше был in32, сейчас int64. Если вернуть int32, воспроизведется. Кажется, что тип данных не должен играть никакую роль, потому что все токкены вписываются даже в int1.

Нужно сказать, что при обучении есть разница между int32 и int64, потому что CELoss не поддерживает int32 и нужно кастить к int64, если тип не тот

Нужно проверить, как лучше с int32 или int64
