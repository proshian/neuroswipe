...
* Думал, что из-за того что стал пропускать токены с нулевой вероятностью, но комментирование этой строчки привело к такому же .csv файлу
* Думал, что из-за того что раньше сохранялись 7 лучших прдсказаний, а сейчас - все. Добавил в aggregate строчку обрезающие предсказания, но тоже не помогло

Нужно отметить, что два пункта выше - это один эксперимент

Для начала хорошо бы понять, что main действительно восрпоизводится. Дальше я бы потихонечку заменял бы куски текущего репозитория версий из  main пока не воспроизведется

Вообще, мне ничего не мешает просто написать в README, что эксперимент воспроизводится в такой-то ветке. Проблема в том, что я не вижу причину, по которой сейчас результаты другие. Это может свидетельствовать о какой-то баге
