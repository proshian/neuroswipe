...
1. Построить и добавить в ридми графики метрик с обучения валидации и метрик бимсерча
      * Для каждой модели скачать чекпоинты, получить из них веса и поставить предсказываться
      * Для каждой модели произвести evaluation
      * Получить график с beamsearch метриками
      * Получить графики всех метрик из tensorboard
2. КОгда сделано все что выше Удалить все модели, кроме семейства v3_models
3. Сделать заметку про очень большой трансформер
4. Сделать заметку про старые модели с соревнования
5. Дальше нужно определить чем отличаются три версии train и сделать одну, или по крайней мере, две, если вариант с гауссовскими фичами слишком сильно отличается
6. Если возможно сделать одну версию predict из predict_v2 и predict_trainable_gaussian
