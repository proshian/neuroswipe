# Note
Если экспериментировать с нуля, то я бы попробовал patience у ReduceLROnPlateau снизить с 20 до хотя бы 15
...
1. Продолжать обучение всех моделей (см google doc) пока не будут выполнены все пункты ниже
2. Обновить README
      * Добавить текст со слайдов из разделов "метрики"
      * Добавить выводы
      * Уточнить заметку про очень большой трансформер (размер)
      ...
      * Заполнить ссылку на диссертацию
3. Убрать файлы про trainable_gaussain в директорию WIP
1. Рефакторинг train.ipynb и predict_v2.py - минимум 30 минут + спросить gpt как бы он улучшил
      * Перенести pl module из jupyter блокнота в python модуль
3. Сделать pull request
2. Попробовать смерджить train.ipynb и train__gaussian.ipynb
3. Попробовать смерджить predict_v2 и predict_trainable_gaussian
4. Добавить логирование lr в train.ipynb
5. Обучить trainable_gaussian
6. Добавить метрики для trainable gaussian (и других моделей)
...
8. Решить, что делать с legacy models... Сделать их v3 + перенос весов / удалить / просто поменять интерфейс (под актуальный датасет) и перенести в отдельный файл?
