...
1. Рефакторинг train.ipynb и predict_v2.py - минимум 60 минут
      * Перенести pl module из jupyter блокнота в python модуль
      * спросить gpt как бы он улучшил
2. Обновить README - ожидаю 60 минут
      * Уточнить заметку про очень большой трансформер (размер)
      * Заполнить TODO
      * Заполнить ссылку на диссертацию
      * Поправить "The sudden metrics improvement" (скорее всего, уже нужно для большего числа)
3. Убрать файлы про trainable_gaussain в директорию WIP - 0 минут)
1. Полный пайплайн для новых датасетов - 120 минут
...
3. Попробовать смерджить predict_v2 и predict_t rainable_gaussian
